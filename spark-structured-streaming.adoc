...
*Spark Structured Streaming* (aka _Spark Streams_) is a stream processing engine in Apache Spark with a <<spark-sql-streaming-Dataset-operators.adoc#, high-level declarative streaming API>> built on top of Spark SQL.
