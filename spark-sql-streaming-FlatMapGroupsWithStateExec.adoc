...
* [[stateFormatVersion]] <<state-format-version, State format version>>
...
* [[batchTimestampMs]] Batch timeout threshold (`batchTimestampMs`)
...
=== [[state-format-version]] State Format Version

When created, `FlatMapGroupsWithStateExec` is given the <<stateFormatVersion, state format version>> that is used exclusively to create the <<stateManager, StateManager>>.

The state format version is based on <<spark-sql-streaming-properties.adoc#spark.sql.streaming.flatMapGroupsWithState.stateFormatVersion, spark.sql.streaming.flatMapGroupsWithState.stateFormatVersion>> internal configuration property (default: `2`).
...
| stateManager
a| [[stateManager]] <<spark-sql-streaming-StateManager.adoc#, StateManager>> for the <<stateEncoder, state encoder>> and the <<isTimeoutEnabled, isTimeoutEnabled>> flag

`StateManager` is created (<<spark-sql-streaming-FlatMapGroupsWithStateExecHelper.adoc#createStateManager, indirectly>>) based on the <<stateFormatVersion, stateFormatVersion>>
