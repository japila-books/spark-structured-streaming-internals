...
`SymmetricHashJoinStateManager` manages join state using the <<keyToNumValues, KeyToNumValuesStore>> and the <<keyWithIndexToValue, KeyWithIndexToValueStore>> state store handlers (and simply acts like a facade)
...
`keyToNumValues` is a <<spark-sql-streaming-KeyToNumValuesStore.adoc#, KeyToNumValuesStore>> that is created immediately for `SymmetricHashJoinStateManager` (for a <<spark-sql-streaming-OneSideHashJoiner.adoc#joinStateManager, OneSideHashJoiner>>).
...
=== [[keyWithIndexToValue]] KeyWithIndexToValueStore -- `keyWithIndexToValue` Internal Property

[source, scala]
----
keyWithIndexToValue: KeyWithIndexToValueStore
----

`keyWithIndexToValue` is a <<spark-sql-streaming-KeyWithIndexToValueStore.adoc#, KeyWithIndexToValueStore>> that is created immediately for `SymmetricHashJoinStateManager` (for a <<spark-sql-streaming-OneSideHashJoiner.adoc#joinStateManager, OneSideHashJoiner>>).

`keyWithIndexToValue` is used when `SymmetricHashJoinStateManager` is requested for the following:
