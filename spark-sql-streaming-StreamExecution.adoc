...
=== [[offsetLog]] Offset Write-Ahead Log -- `offsetLog` Property
...
`offsetLog` is <<spark-sql-streaming-OffsetSeqLog.adoc#, Offset Write-Ahead Log>> with `offsets` <<checkpointFile, metadata checkpoint directory>> to persist offsets (offset range) of the data about to be processed in every trigger.
