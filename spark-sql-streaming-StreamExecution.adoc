...
CAUTION: FIXME A picture with different time durations for "time takens" on a single streaming batch's timeline.
...
`constructNextBatch` commits the offsets for the batch (only when <<currentBatchId, currentBatchId>> is non-``0``, i.e. ...FIXME).

CAUTION: FIXME When is `currentBatchId` not `0`?
