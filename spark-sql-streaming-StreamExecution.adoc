...
`StreamExecution` is the <<contract, base>> of <<extensions, stream execution engines>> (aka _streaming query processing engines_) that <<runActivatedStream, run>> an activated <<logicalPlan, structured query>> (on a <<queryExecutionThread, stream execution thread>>).
...
`availableOffsets` is a <<spark-sql-streaming-StreamProgress.adoc#, collection of offsets per streaming source>> to track what data (by <<spark-sql-streaming-Offset.adoc#, offset>>) is available to be processed for every <<spark-sql-streaming-ProgressReporter.adoc#sources, streaming source>> in the <<analyzedPlan, streaming query>> but have not yet been committed (and registered as <<committedOffsets, committed>>).
