...
1. <<runBatch-triggerLogicalPlan, Transforming Catalyst Expressions>>
1. <<runBatch-queryPlanning, Creating IncrementalExecution for Current Streaming Batch>>
...
==== [[runBatch-getBatch]] Requesting New (and Hence Unprocessed) Data From Streaming Sources (getBatch Phase)
...
==== [[runBatch-withNewSources]] Replacing StreamingExecutionRelations (in Logical Plan) With Relations With New Data or Empty LocalRelation (withNewSources Phase)
...
==== [[runBatch-triggerLogicalPlan]] Transforming Catalyst Expressions (triggerLogicalPlan Phase)
...
NOTE: `CurrentTimestamp` Catalyst expression corresponds to `current_timestamp` function. You can find more about `current_timestamp` function in https://jaceklaskowski.gitbooks.io/mastering-apache-spark/spark-sql-functions-datetime.html#current_timestamp[Mastering Apache Spark 2] gitbook.

NOTE: `CurrentDate` Catalyst expression corresponds to `current_date` function.

==== [[runBatch-queryPlanning]] Creating IncrementalExecution for Current Streaming Batch (queryPlanning Phase)
