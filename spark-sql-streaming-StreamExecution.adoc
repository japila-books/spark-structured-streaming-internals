...
`StreamExecution` represents the execution environment to execute a streaming query continuously (every <<trigger, trigger>>) and write the result to a <<sink, streaming sink>>.
