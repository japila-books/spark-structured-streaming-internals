...
`constructNextBatch` link:spark-sql-streaming-ProgressReporter.adoc#reportTimeTaken[reports the time taken] for link:spark-sql-streaming-Source.adoc#getOffset[getting the offset per source].
...
==== [[runBatch-getBatch]] Requesting Unprocessed Data From Streaming Sources

Internally, `runBatch` first requests the link:spark-sql-streaming-Source.adoc[streaming sources] for unprocessed data (as `newData`).
