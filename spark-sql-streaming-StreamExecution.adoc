...
Initialized right after `StreamExecution` starts <<runBatches, running batches>> (which is when <<microBatchThread, stream execution thread>> is started).
...
NOTE: `constructNextBatch` uses the `Source` contract to link:spark-sql-streaming-Source.adoc#getOffset[get the latest offset].

`constructNextBatch` <<updateStatusMessage, updates status message>> to the following for every streaming data source.
...
When new data is available, `constructNextBatch` updates the eventTime watermark (tracked using <<offsetSeqMetadata, offsetSeqMetadata>>) if it finds one in the <<lastExecution, last IncrementalExecution>>.

If <<lastExecution, lastExecution>> is available (which may not when `constructNextBatch` is executed the very first time), `constructNextBatch` takes the executed physical plan (i.e. `SparkPlan`) to collect all `EventTimeWatermarkExec` physical operators with the count of link:spark-sql-streaming-EventTimeWatermarkExec.adoc#eventTimeStats[eventTimeStats] greater than `0`.
...
```
DEBUG Observed event time stats: [eventTimeStats]
```
...
`constructNextBatch` then takes the first difference (if available at all) and uses it as a possible new eventTime watermark.

If the eventTime watermark candidate is greater than the current watermark (i.e. later time-wise), `constructNextBatch` prints out the following INFO message to the logs:

```
INFO Updating eventTime watermark to: [newWatermarkMs] ms
```

`constructNextBatch` creates a new <<offsetSeqMetadata, OffsetSeqMetadata>> with the new eventTime watermark and the current time.
...
```
DEBUG Event time didn't move: [newWatermarkMs] <= [batchWatermarkMs]
```
...
CAUTION: FIXME When could a physical plan have two or more `EventTimeWatermarkExec` operators?
...
CAUTION: FIXME Example of the above difference

`constructNextBatch` then adds the offsets to metadata log.

CAUTION: FIXME Describe the writing (after `updateStatusMessage("Writing offsets to log")`)

`constructNextBatch` commits the offsets for the batch.

CAUTION: FIXME Describe the committing (after `val prevBatchOff = offsetLog.get(currentBatchId - 1)`)

In the end, `constructNextBatch` purges <<offsetLog, offsetLog>> and <<batchCommitLog, batchCommitLog>>.

CAUTION: FIXME Describe the purges (i.e. `if (minBatchesToRetain < currentBatchId)`)
...
If <<state, state>> is not `TERMINATED` (which means that the streaming query should be executed), `batchRunner`  executes the current batch and link:spark-sql-streaming-ProgressReporter.adoc#reportTimeTaken[reports the time taken].
...
When no batch if and metadata were available, it is assumed that the streaming query is started for the first time. You should see the following INFO message in the logs:
