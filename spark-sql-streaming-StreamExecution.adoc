...
| Unique link:spark-sql-streaming-Source.adoc[streaming Sources] (in the <<logicalPlan, logical query plan>>).
...
`constructNextBatch` starts by checking whether a new data is available across the streaming sources (in the <<logicalPlan, logical query plan>>).

`constructNextBatch` first acquires <<awaitBatchLock, awaitBatchLock>> and gets the offsets for <<uniqueSources, every streaming source used>>.

NOTE: `constructNextBatch` checks out the available offsets in every streaming source sequentially, i.e. one by one.
...
`constructNextBatch` link:spark-sql-streaming-Source.adoc#getOffset[gets the current offset from every streaming source] and link:spark-sql-streaming-ProgressReporter.adoc#reportTimeTaken[reports the time taken].
...
`constructNextBatch` adds the streaming sources with the available offsets to <<availableOffsets, availableOffsets>>.
...
Firstly, `constructNextBatch` updates the eventTime watermark (tracked using <<offsetSeqMetadata, offsetSeqMetadata>>) if we find one in the <<lastExecution, last IncrementalExecution>>.

CAUTION: FIXME Describe the eventTime watermark update
...
CAUTION: FIXME Describe the writing
...
CAUTION: FIXME Describe the purges
