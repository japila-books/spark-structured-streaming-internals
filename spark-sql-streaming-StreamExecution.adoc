...
`committedOffsets` is a <<spark-sql-streaming-StreamProgress.adoc#, collection of offsets per streaming source>> to track what data (<<spark-sql-streaming-Offset.adoc#, offset>>) has already been processed and committed (to a sink or state stores) for every streaming source.
...
* `ContinuousExecution` stream execution engine is requested for the <<spark-sql-streaming-ContinuousExecution.adoc#getStartOffsets, start offsets (from checkpoint)>> and to <<spark-sql-streaming-ContinuousExecution.adoc#commit, commit epoch>>
