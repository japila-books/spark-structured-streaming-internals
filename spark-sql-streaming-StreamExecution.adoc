...
When <<start, started>>, `StreamExecution` starts a <<queryExecutionThread, query execution thread>> that simply <<runStream, runs stream processing>> (and hence the streaming query).
