...
a| [[queryExecutionThread]] *Stream execution thread*

The thread of execution to run a streaming query concurrently with the name as `stream execution thread for [id]` (that uses <<prettyIdString, prettyIdString>> for logging purposes).
