...
[[minLogEntriesToMaintain]]
[[spark.sql.streaming.minBatchesToRetain]]
`StreamExecution` uses...FIXME
...
`StreamExecution` uses *__is_continuous_processing* local property to...FIXME
...
| commitLog
a| [[commitLog]] *Commit Log* (<<spark-sql-streaming-CommitLog.adoc#, CommitLog>> with `commits` <<checkpointFile, metadata checkpoint directory>>) for completed streaming batches (with a single file per batch with a file name being the batch id) or committed epochs.
