<<awaitProgressLock, == [[StreamExecution]] StreamExecution -- Base of Streaming Query Executions

`StreamExecution` is the <<contract, base>> of <<extensions, streaming query executions>> that can execute <<logicalPlan, structured query>> continuously.
...
a| [[queryExecutionThread]] Thread of execution to run a streaming query concurrently with the name as `stream execution thread for [prettyIdString]` (that uses <<prettyIdString, prettyIdString>> for logging purposes).

When started, `queryExecutionThread` sets the so-called call site and <<runBatches, runs streaming batches>>.

NOTE: `queryExecutionThread` is Java's https://docs.oracle.com/javase/8/docs/api/java/lang/Thread.html[java.util.Thread].
...
`start` then sets the <<queryExecutionThread, queryExecutionThread>> as a daemon thread and starts it.
