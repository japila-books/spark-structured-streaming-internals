...
`StreamExecution` represents the execution environment for continuous execution of a streaming query (every <<trigger, trigger>>) and writing the result to a <<sink, streaming sink>>.

NOTE: The streaming query is a streaming `Dataset` that is described using <<logicalPlan, analyzed logical plan>>.
