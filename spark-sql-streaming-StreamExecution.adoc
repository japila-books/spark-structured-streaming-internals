...
| id
a| [[id]] Unique identifier of the streaming query
...
| streamMetadata
| [[streamMetadata]] <<spark-sql-streaming-StreamMetadata.adoc#, StreamMetadata>> from the `metadata` file from the <<checkpointFile, checkpoint directory>>. If the `metadata` file is not available it is created (with a new random <<id, id>>).
