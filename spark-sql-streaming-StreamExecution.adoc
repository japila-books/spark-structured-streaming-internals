...
`runBatch` records the output schema of the streaming source from `StreamingExecutionRelation` and the `DataFrame` with the new data (in `replacements` internal buffer).

`runBatch` transforms Catalyst expressions in `withNewSources` new logical plan.
