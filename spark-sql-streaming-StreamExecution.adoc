...
* Incremented when `StreamExecution` <<runBatches, runs streaming batches>> and finishes a trigger with <<dataAvailable, data available in sources>> right after committing the batch.
...
* `StreamExecution` <<runBatches, runs streaming batches>> (and announces starting a streaming query by posting `QueryStartedEvent` and query termination by posting `QueryTerminatedEvent`)
====
