...
`StreamExecution` is the <<contract, base>> of <<extensions, streaming query executions>> that can execute <<logicalPlan, structured queries>> continuously.
...
Used exclusively when `StreamExecution` is requested to <<runStream, runStream>> for the very first time (when transitioning from `INITIALIZING` state to `ACTIVE`)
|===
...
| `commitLog`
| [[commitLog]] `CommitLog` for the <<sparkSession, SparkSession>> and *commits* <<checkpointFile, checkpointFile>>
...
In *awaitBatchLock* code block (it is not a time-tracking section), `runBatch` acquires a lock on <<awaitBatchLock, awaitBatchLock>>, wakes up all waiting threads on <<awaitBatchLockCondition, awaitBatchLockCondition>> and immediatelly releases <<awaitBatchLock, awaitBatchLock>> lock.

NOTE: <<awaitBatchLockCondition, awaitBatchLockCondition>> is used mainly when `StreamExecution` <<processAllAvailable, processAllAvailable>> (and also when `awaitOffset`, but that seems mainly for testing).
...
`processAllAvailable` acquires a lock on <<awaitBatchLock, awaitBatchLock>> and turns <<noNewData, noNewData>> flag off.

`processAllAvailable` keeps waiting 10 seconds for <<awaitBatchLockCondition, awaitBatchLockCondition>> until <<noNewData, noNewData>> flag is turned on or `StreamExecution` is no longer <<isActive, active>>.
...
In the end, `processAllAvailable` releases <<awaitBatchLock, awaitBatchLock>> lock.

=== [[runStream]] `runStream` Internal Method
...
NOTE: `runStream` is used when...FIXME
