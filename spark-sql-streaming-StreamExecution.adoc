...
Used for <<logicalPlan, logicalPlan>> (while transforming <<analyzedPlan, analyzedPlan>> and replacing `StreamingRelation` logical operator to a corresponding `StreamingExecutionRelation` with the streaming data source created passing in the path to `sources` directory to store checkpointing metadata).
...
NOTE: A physical plan has as many `EventTimeWatermarkExec` physical operators as link:spark-sql-streaming-Dataset-operators.adoc#withWatermark[withWatermark] operator was used to create a streaming query.
