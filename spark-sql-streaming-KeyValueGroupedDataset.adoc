...
`KeyValueGroupedDataset` allows for *streaming aggregation* and represents a *grouped dataset* as a result of `Dataset.groupByKey` operator (that groups rows given condition defined by a grouping function).
...
`KeyValueGroupedDataset` shines the most when used with streaming Datasets.
