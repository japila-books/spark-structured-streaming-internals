...
`runBatch` transforms the <<logicalPlan, analyzed logical plan>> to include <<runBatch-getBatch, Sources and MicroBatchReaders with new data>> (i.e. data that has arrived since the last batch).
...
* Transformed <<logicalPlan, logical query plan>> with <<runBatch-newBatchesPlan, logical relations>> for every streaming source and reader (and <<runBatch-triggerLogicalPlan, corresponding attributes>>)
