...
NOTE: `constructNextBatch` can only be executed when the <<isCurrentBatchConstructed, isCurrentBatchConstructed>> internal flag is enabled (`true`).

`constructNextBatch` firstly requests the <<spark-sql-streaming-StreamExecution.adoc#uniqueSources, streaming sources>> for their latest available offset.

NOTE: FIXME: Describe me (`latestOffsets`)
...
`constructNextBatch` checks whether or not the next streaming micro-batch should be constructed (`lastExecutionRequiresAnotherBatch`). `constructNextBatch` uses the very last <<spark-sql-streaming-StreamExecution.adoc#lastExecution, IncrementalExecution>> if the <<spark-sql-streaming-IncrementalExecution.adoc#shouldRunAnotherBatch, last execution requires another micro-batch>> (using the <<spark-sql-streaming-StreamExecution.adoc#offsetSeqMetadata, batch metadata>>) and the given `noDataBatchesEnabled` flag is enabled (`true`). `constructNextBatch` also <<isNewDataAvailable, checks out whether new data is available (per available and committed offsets)>>.
...
`constructNextBatch` branches off per whether to <<constructNextBatch-shouldConstructNextBatch-enabled, constructs>> or <<constructNextBatch-shouldConstructNextBatch-disabled, skip>> the next batch (`shouldConstructNextBatch` in the above TRACE message).

In the end, `constructNextBatch` returns the value of the `shouldConstructNextBatch` flag.

==== [[constructNextBatch-shouldConstructNextBatch-enabled]] Constructing Next Batch (`shouldConstructNextBatch` Flag Enabled)

NOTE: FIXME (`if (shouldConstructNextBatch) ...`)

==== [[constructNextBatch-shouldConstructNextBatch-disabled]] Skipping Next Batch (`shouldConstructNextBatch` Flag Disabled)

NOTE: FIXME (`if (shouldConstructNextBatch) ... else ...`)
