...
. <<populateStartOffsets, Populating start offsets (from metadata logs)>> at the very first batch

. <<constructNextBatch, Constructing the next streaming micro-batch>>

. <<runBatch, Running the streaming micro-batch>>

While <<populateStartOffsets, populating start offsets (from the offsets and commits metadata logs)>> at the very first batch the batch runner prints out the following INFO message to the logs:
