...
In *getBatch* <<spark-sql-streaming-ProgressReporter.adoc#reportTimeTaken, time-tracking section>>, `runBatch` goes over the <<spark-sql-streaming-StreamExecution.adoc#availableOffsets, available offsets>> and...FIXME

==== [[runBatch-getBatch-Source]] getBatch Phase and Streaming Sources
...
In case the `Source` gives a dataframe that is not streaming, `runBatch` throws an `AssertionError`:

```
DataFrame returned by getBatch from [source] did not have isStreaming=true\[nlogicalQueryPlan]
```

==== [[runBatch-getBatch-MicroBatchReader]] getBatch Phase and MicroBatchReaders

For a <<spark-sql-streaming-MicroBatchReader.adoc#, MicroBatchReader>> (with the available <<spark-sql-streaming-Offset.adoc#, offsets>> different from the <<spark-sql-streaming-StreamExecution.adoc#committedOffsets, committedOffsets>> registry), `runBatch`...FIXME
...
Find out more about `current_timestamp` function in https://jaceklaskowski.gitbooks.io/mastering-spark-sql/spark-sql-functions-datetime.html#current_timestamp[The Internals of Spark SQL] gitbook.

---

`CurrentDate` Catalyst expression corresponds to `current_date` function.

Find out more about `current_date` function in https://jaceklaskowski.gitbooks.io/mastering-spark-sql/spark-sql-functions-datetime.html#current_date[The Internals of Spark SQL] gitbook.
====
