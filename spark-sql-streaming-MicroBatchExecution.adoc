== [[MicroBatchExecution]] MicroBatchExecution -- StreamExecution in Micro-Batch Stream Processing

`MicroBatchExecution` is a <<spark-sql-streaming-StreamExecution.adoc#, StreamExecution>> in <<spark-sql-streaming-micro-batch-processing.adoc#, Micro-Batch Stream Processing>>.

`MicroBatchExecution` is <<creating-instance, created>> when `StreamingQueryManager` is requested to <<spark-sql-streaming-StreamingQueryManager.adoc#createQuery, create a streaming query>> with any type of <<sink, sink>> but <<spark-sql-streaming-StreamWriteSupport.adoc#, StreamWriteSupport>> and any type of <<trigger, trigger>> but <<spark-sql-streaming-Trigger.adoc#ContinuousTrigger, ContinuousTrigger>> (when `DataStreamWriter` is requested to <<spark-sql-streaming-DataStreamWriter.adoc#start, start an execution of the streaming query>>).
...
=== [[logicalPlan]] Initializing Analyzed Logical Plan of Streaming Query -- `logicalPlan` Property
...
NOTE: `logicalPlan` is part of <<spark-sql-streaming-StreamExecution.adoc#logicalPlan, StreamExecution Contract>> to initialize the analyzed logical plan of the streaming query.
