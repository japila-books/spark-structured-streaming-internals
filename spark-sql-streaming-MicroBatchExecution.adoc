...
`runActivatedStream` simply requests the <<triggerExecutor, TriggerExecutor>> to execute batches using the <<batchRunner, batch runner>>.

==== [[batchRunner]] Batch Runner

The batch runner (of the <<triggerExecutor, TriggerExecutor>>) is executed as long as the  <<spark-sql-streaming-StreamExecution.adoc#isActive, is active>>
...
`stop` sets the <<spark-sql-streaming-StreamExecution.adoc#state, state>> to `TERMINATED`.
