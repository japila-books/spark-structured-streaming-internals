...
`ContinuousExecution` is a <<spark-sql-streaming-StreamExecution.adoc#, StreamExecution>> with exactly one <<sink, StreamWriteSupport sink>> and a <<trigger, ContinuousTrigger>>.
