...
[NOTE]
====
The trick here is that the top-level operator in the physical query plan is always a <<spark-sql-streaming-WriteToContinuousDataSourceExec.adoc#, WriteToContinuousDataSourceExec>> and so requesting it to <<spark-sql-streaming-WriteToContinuousDataSourceExec.adoc#doExecute, doExecute>> leads to requesting the underlying <<spark-sql-streaming-ContinuousWriteRDD.adoc#, ContinuousWriteRDD>> to collect.

That collect operator is how a Spark job is run (on all partitions in an RDD) as described by the <<spark-sql-streaming-ContinuousWriteRDD.adoc#compute, ContinuousWriteRDD.compute>> method.

While computing a partition, `ContinuousWriteRDD` <<spark-sql-streaming-ContinuousWriteRDD.adoc#compute-loop, is never meant to stop unless a task is killed or completed>>. The Spark job with the tasks ran on executors simply runs indefinitely.
====
