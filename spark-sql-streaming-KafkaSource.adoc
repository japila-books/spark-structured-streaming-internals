...
Lazily set when `KafkaSource` <<getOffset, gets maximum available offset>> and used for <<getBatch, generating a DataFrame with records from Kafka for a batch>>.
...
`getBatch` requests `KafkaSourceOffset` to link:spark-sql-streaming-KafkaSourceOffset.adoc#getPartitionOffsets[give partition offsets] for the input `end` offset (known as `untilPartitionOffsets`).

`getBatch` requests `KafkaSourceOffset` to link:spark-sql-streaming-KafkaSourceOffset.adoc#getPartitionOffsets[give partition offsets] for the input `start` offset if defined or uses <<initialPartitionOffsets, initialPartitionOffsets>> (known as `fromPartitionOffsets`).
...
```
DEBUG TopicPartitions: [comma-separated topicPartitions]
```
...
```
DEBUG Sorted executors: [comma-separated sortedExecutors]
```
...
```
INFO GetBatch generating RDD of offset range: [comma-separated offsetRanges sorted by topicPartition]
```
