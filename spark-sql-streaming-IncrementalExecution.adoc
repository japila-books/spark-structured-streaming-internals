...
[[preparations]]
`IncrementalExecution` registers <<state, state>> physical optimization rule with the parent ``QueryExecution``'s `preparations` that prepares the streaming physical plan (using iteration-specific execution properties).
