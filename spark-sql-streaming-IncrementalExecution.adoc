...
`shouldRunAnotherBatch` is enabled (`true`) if there is at least one <<spark-sql-streaming-StateStoreWriter.adoc#, StateStoreWriter>> operator (in the <<executedPlan, executedPlan physical query plan>>) that <<spark-sql-streaming-StateStoreWriter.adoc#shouldRunAnotherBatch, requires another non-data batch>> (per the given <<spark-sql-streaming-OffsetSeqMetadata.adoc#, OffsetSeqMetadata>> with the event-time watermark and the batch timestamp). Otherwise, `shouldRunAnotherBatch` is disabled (`false`).
