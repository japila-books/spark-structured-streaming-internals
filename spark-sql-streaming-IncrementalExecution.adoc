...
`IncrementalExecution` is a `QueryExecution` of a streaming Dataset that link:spark-sql-streaming-StreamExecution.adoc#runBatch-queryPlanning[StreamExecution] creates when incrementally executing the <<logicalPlan, logical query plan>> (every trigger).
