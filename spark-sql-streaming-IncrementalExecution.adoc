...
val checkpointLocation = s"checkpoint-$queryName"
val query = spark
  .readStream
  .format("rate")
  .load
...
assert(stateCheckpointDir endsWith stateDir)
----
...
=== [[numStateStores]] Number of State Stores (Shuffle Partitions) -- `numStateStores` Internal Property
