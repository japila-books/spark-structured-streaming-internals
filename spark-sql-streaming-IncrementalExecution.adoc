...
`IncrementalExecution` is the `QueryExecution` of a <<logicalPlan, streaming query>>.
...
=== [[planner]][[extraPlanningStrategies]] SparkPlanner -- `planner` Property

As a custom `QueryExecution`, `IncrementalExecution` uses a custom execution planning strategies that are used to plan the <<logicalPlan, streaming query>> for execution:
