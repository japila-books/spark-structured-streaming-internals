...
`StreamingSymmetricHashJoinExec` is a binary physical operator that represents a <<spark-sql-streaming-join.adoc#, Streaming Join>> (a `Join` logical operator of two streaming queries at execution time).
