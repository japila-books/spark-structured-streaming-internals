...
`StreamingSymmetricHashJoinExec` is a binary physical operator for a <<spark-sql-streaming-join.adoc#, stream-stream join>> (a `Join` logical operator of two streaming queries at execution time).
