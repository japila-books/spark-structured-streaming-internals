...
`FlatMapGroupsWithStateStrategy` is an execution planning strategy (i.e. `Strategy`) that `SparkPlanner` uses to <<apply, plan>> `FlatMapGroupsWithState` logical operators.
