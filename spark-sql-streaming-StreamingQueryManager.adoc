...
* Set in <<notifyQueryTermination, notifyQueryTermination>> when `StreamExecution` link:spark-sql-streaming-StreamExecution.adoc#runBatches-finally[has finished (running streaming batches)] (possibly due to an exception).
