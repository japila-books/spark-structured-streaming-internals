...
NOTE: `startTrigger` is used exclusively when <<triggerExecutor, TriggerExecutor>> executes a batch runner (as part of `StreamExecution` link:spark-sql-streaming-StreamExecution.adoc#runBatches[running batches]).
