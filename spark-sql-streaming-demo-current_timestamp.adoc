...
Both are special for Spark Structured Streaming as `StreamExecution` link:spark-sql-streaming-StreamExecution.adoc#runBatch-triggerLogicalPlan[replaces] their underlying Catalyst expressions, `CurrentTimestamp` and `CurrentDate` respectively, with `CurrentBatchTimestamp` expression and the time of the current batch.
