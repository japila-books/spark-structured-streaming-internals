...
`EventTimeWatermarkExec` is a unary physical operator that <<doExecute, accumulates>> the event time values (that appear in the <<eventTime, eventTime watermark column>>).
