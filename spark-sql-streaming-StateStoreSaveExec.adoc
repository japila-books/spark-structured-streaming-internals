...
NOTE: You can see the current value as `numRowsUpdated` attribute in `stateOperators` in <<spark-sql-streaming-StreamingQueryProgress.adoc#, StreamingQueryProgress>> (that is available as `StreamingQuery.lastProgress.stateOperators(n).numRowsUpdated` for ``n``th operator).
...
TIP: Refer to <<spark-sql-streaming-StateStoreSaveExec-Append.adoc#, Demo: StateStoreSaveExec with Append Output Mode>> for an example of `StateStoreSaveExec` with `Append` output mode.
