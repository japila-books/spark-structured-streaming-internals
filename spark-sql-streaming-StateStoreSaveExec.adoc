...
== Physical Plan ==
*Sort [group#177 ASC NULLS FIRST], true, 0
+- Exchange rangepartitioning(group#177 ASC NULLS FIRST, 1)
   +- ObjectHashAggregate(keys=[group#177], functions=[collect_list(value#188, 0, 0)])
      +- Exchange hashpartitioning(group#177, 1)
         +- StateStoreSave [group#177], StatefulOperatorStateInfo(<unknown>,128a7226-931f-41b5-a52f-10d47c5c5571,0,0), Append, 0
            +- ObjectHashAggregate(keys=[group#177], functions=[merge_collect_list(value#188, 0, 0)])
               +- Exchange hashpartitioning(group#177, 1)
                  +- StateStoreRestore [group#177], StatefulOperatorStateInfo(<unknown>,128a7226-931f-41b5-a52f-10d47c5c5571,0,0)
                     +- ObjectHashAggregate(keys=[group#177], functions=[merge_collect_list(value#188, 0, 0)])
                        +- Exchange hashpartitioning(group#177, 1)
                           +- ObjectHashAggregate(keys=[group#177], functions=[partial_collect_list(value#188, 0, 0)])
                              +- *Project [split(cast(value#153 as string), ,)[0] AS group#177, cast(split(cast(value#153 as string), ,)[1] as int) AS value#188]
                                 +- StreamingRelation kafka, [key#152, value#153, topic#154, partition#155, offset#156L, timestamp#157, timestampType#158]
