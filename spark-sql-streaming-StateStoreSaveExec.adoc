...
`StateStoreSaveExec` is a unary physical operator that <<spark-sql-streaming-StateStoreWriter.adoc#, writes a streaming state to a state store>> with <<spark-sql-streaming-WatermarkSupport.adoc#, support for streaming watermark>>.
...
=== [[demo]] Demo: StateStoreSaveExec Operator in Physical Plan of Streaming Aggregation (Dataset.groupBy Operator)
...
* [[eventTimeWatermark]] Execution-specific event-time watermark (default: `None`)
