...
`StatefulAggregationStrategy` is an execution planning strategy that can <<apply, plan streaming queries>> with `EventTimeWatermark` and `Aggregate` logical operators only:

* <<spark-sql-streaming-EventTimeWatermark.adoc#, EventTimeWatermark>> logical operator represents <<spark-sql-streaming-Dataset-operators.adoc#withWatermark, Dataset.withWatermark>> operator in a logical query plan.

* `Aggregate` logical operator represents <<spark-sql-streaming-Dataset-operators.adoc#groupBy, Dataset.groupBy>> and <<spark-sql-streaming-Dataset-operators.adoc#groupByKey, Dataset.groupByKey>> operators and SQL's `GROUP BY` clause in a logical query plan.
