...
* `FlatMapGroupsWithStateExec` physical operator is <<spark-sql-streaming-FlatMapGroupsWithStateExec.adoc#doExecute, executed>> (after all rows in a partition have been processed successfully)

* `StreamingDeduplicateExec` physical operator is <<spark-sql-streaming-StreamingDeduplicateExec.adoc#doExecute, doExecute>> (after all rows in a partition have been processed successfully)

* `StreamingGlobalLimitExec` physical operator is <<spark-sql-streaming-StreamingGlobalLimitExec.adoc#doExecute, doExecute>> (after all rows in a partition have been processed successfully)

* `StreamingAggregationStateManagerBaseImpl` is requested to <<spark-sql-streaming-StreamingAggregationStateManagerBaseImpl.adoc#commit, commit (changes to) a state store>>
