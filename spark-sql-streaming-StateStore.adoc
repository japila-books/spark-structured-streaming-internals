...
| _coordRef
| [[_coordRef]] <<spark-sql-streaming-StateStoreCoordinatorRef.adoc#, StateStoreCoordinatorRef>> (a `RpcEndpointRef` to link:spark-sql-streaming-StateStoreCoordinator.adoc[StateStoreCoordinator]).
...
=== [[coordinatorRef]] Retrieving StateStoreCoordinatorRef (for Executors) -- `coordinatorRef` Internal Object Method
