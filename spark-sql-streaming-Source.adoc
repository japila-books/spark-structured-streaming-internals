== [[Source]] Source Contract -- Streaming Sources

*Streaming Source* is a "continuous" stream of data and is described using the <<contract, Source Contract>>.
...
Latest (maximum available) <<spark-sql-streaming-Offset.adoc#, offset>> (or `None` when never received any data)
