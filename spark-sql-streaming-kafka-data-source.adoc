...
With the `spark-sql-kafka-0-10` module on CLASSPATH you can use *kafka* data source format for loading streaming datasets:
...
NOTE: The *kafka* data source format is available through <<spark-sql-streaming-KafkaSourceProvider.adoc#, KafkaSourceProvider>> that is distributed as part of the `spark-sql-kafka-0-10` module.

=== Stream Processing Mode

Kafka Data Source can be used in streaming and batch processing modes, in Spark Structured Streaming and Spark SQL, respectively.
...
[NOTE]
====
The fixed schema is defined as part of the `DataSourceReader` contract (through <<spark-sql-streaming-MicroBatchReader.adoc#, MicroBatchReader>> and <<spark-sql-streaming-ContinuousReader.adoc#, ContinuousReader>> extension contracts).

Read up on https://jaceklaskowski.gitbooks.io/mastering-spark-sql/spark-sql-DataSourceReader.html[DataSourceReader Contract] in https://bit.ly/mastering-spark-sql[The Internals of Spark SQL] book.
====
