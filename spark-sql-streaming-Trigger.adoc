...
`Trigger` defines how often a link:spark-sql-streaming-StreamingQuery.adoc[streaming query] should be executed and therefore produce results.
...
| `ProcessingTime(Duration interval)`
...
NOTE: Although `Trigger` allows for custom implementations, `StreamExecution` will refuse them and report an `IllegalStateException`.
